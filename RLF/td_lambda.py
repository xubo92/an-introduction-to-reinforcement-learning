...
class Temporal_Difference_lambda:
    ...
    def naive_Q_lambda(self,agent,episode_num,epsilon,alpha,gamma,Lambda,max_timestep,eval_interval):
        ...
        while ep_idx < episode_num:
            ...
            while not (agent.isTerminated() or n >= max_timestep):
                ...
                next_action_idx = np.random.choice(self.action_num, 1, p=self.mu[agent.next_state])[0]
                next_best_action_idx = np.argmax(self.Q[agent.next_state])

                delta = agent.c_reward + gamma * self.Q[agent.next_state][next_best_action_idx] - self.Q[agent.c_state][next_action_idx]
                self.Z[agent.c_state][agent.c_action] += 1
